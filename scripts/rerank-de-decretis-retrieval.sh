...
QUERY="${1:-Why did the Council use the expressions from the essence and one in essence?}"
MODE="${2:-exact}"
TOP_K="${3:-5}"
CANDIDATE_SEGMENT_K="${4:-10}"
...
if [[ -z "${APOLOGIA_KNOWLEDGE_DB_PASSWORD:-}" ]]; then
  ...
  APOLOGIA_KNOWLEDGE_DB_PASSWORD="$(sed -n 's/^APOLOGIA_KNOWLEDGE_DB_PASSWORD=//p' "${ENV_FILE}" | tail -n 1)"
fi
[[ -n "${APOLOGIA_KNOWLEDGE_DB_PASSWORD:-}" ]] || fail "APOLOGIA_KNOWLEDGE_DB_PASSWORD is not configured."
...
ollama list | awk 'NR > 1 { print $1 }' | grep -Fxq "${EMBEDDING_MODEL}" || fail "Required embedding model ${EMBEDDING_MODEL} is unavailable."
ollama list | awk 'NR > 1 { print $1 }' | grep -Fxq "${RERANKER_MODEL}" || fail "Required reranker model ${RERANKER_MODEL} is unavailable."
...
dotnet run --project "${PROJECT}" -- \
  ...
  --mode "${MODE}" \
  --top-k "${TOP_K}" \
  --candidate-segment-k "${CANDIDATE_SEGMENT_K}"
